    ...
    <Card className="flext flex-col bg-muted/50">
      <CardHeader className="flex-none p-4">
        <CardTitle className="flex items-center justify-between text-sm font-medium">
          {column.title}
          <span className="rounded-full px-2 py-1 text-xs bg-[#BA532D]">{tasks.length}</span>
        </CardTitle>
      </CardHeader>
      <CardContent
      ...
      <div className="flex-none p-2">
        {showAddTask ? (
          ...
          <Button variant="ghost" className=" border-t-2  border-[#BA532D] w-full justify-start" onClick={() => setShowAddTask(true)}>
